...
pub async fn run_once(data_dir: &Path, config: &Config) -> Result<()> {
    ...
    let l1_for_diff = l1_remote.clone();
    let l2_for_diff = l2_remote.clone();
    let (delta, total_changes) = tokio::task::spawn_blocking(move || diff(&l1_local, &l2_local, &l1_for_diff, &l2_for_diff))
        .await.context("Diffing and deduplication failed")?;

    let timestamp = Utc::now().format("%Y%m%d%H%M%S");
    fs::write(data_dir.join(format!("delta-{timestamp}.txt")), delta).await?;
    tokio::try_join!(
    ...
    info!("Total changes: {total_changes}");
    etags.l1 = l1_etag;
    etags.l2 = l2_etag;
...
fn diff(l1_local: &str, l2_local: &str, l1_remote: &str, l2_remote: &str) -> (String, usize) {
    ...
    for value in additions { output.push_str(&format!("+ {value}\n")); }
    for value in deletions { output.push_str(&format!("- {value}\n")); }
    (output, total)
}
