...
fn init_logging() -> Result<()> {
    let log_dir = Path::new("/var/log/firehol");
    fs::create_dir_all(log_dir).context("Failed to create /var/log/firehol")?;
    let file = fs::OpenOptions::new()
        .create(true)
        .append(true)
        .open(log_dir.join("firehol.log"))
        .context("Failed to open /var/log/firehol/firehol.log")?;
